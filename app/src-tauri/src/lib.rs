...
/// Whether desktop notifications for incoming Telegram messages are enabled.
/// Defaults to on; the frontend syncs this from its persisted setting via
/// `cmd_set_notifications_enabled` (see `commands/settings.rs`).
pub struct NotificationSettings(pub Arc<std::sync::atomic::AtomicBool>);
...
pub fn run() {
    ...
    let app = builder
        .setup(move |app| {
                            ...
                            webview.jni_handle().exec(|env, context, _webview| {
                                ...
                                if let Ok(class_loader_val) = env.call_method(
                                    ...
                                    if let Ok(class_loader_obj) = class_loader_val.l() {
                                        ...
                                        let class_name_jstr = match env.new_string("com.wana84.yamdrive.MainActivity") {
                                            Ok(s) => Some(s),
                                            Err(e) => {
            ...
            app.manage(TelegramState {
                ...
                notifier_shutdown: Arc::new(std::sync::Mutex::new(None)),
            ...
            app.manage(NotificationSettings(Arc::new(std::sync::atomic::AtomicBool::new(true))));
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::cmd_get_self_profile,
            commands::cmd_set_notifications_enabled,
        ])
    ...
    app.run(|app_handle, event| {
        if let tauri::RunEvent::Exit = event {
            ...
            let notifier_arc = app_handle.state::<TelegramState>().notifier_shutdown.clone();
            let notifier_tx = notifier_arc.lock().ok().and_then(|mut g| g.take());
            if let Some(tx) = notifier_tx {
                log::info!("Signaling notification listener shutdown...");
                let _ = tx.send(());
            }
