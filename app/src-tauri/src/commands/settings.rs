...
/// Enable/disable desktop notifications for incoming Telegram messages.
/// Called by the frontend whenever the "Appearance"/"Language & Region"
/// settings section's notification toggle changes, and once on startup so
/// the Rust-side listener (`spawn_notification_listener` in `commands/auth.rs`)
/// reflects the persisted preference from `settings.json`.
#[tauri::command]
pub async fn cmd_set_notifications_enabled(
    enabled: bool,
    notification_settings: State<'_, NotificationSettings>,
) -> Result<(), String> {
    notification_settings
        .0
        .store(enabled, std::sync::atomic::Ordering::Relaxed);
    log::info!("Message notifications {}", if enabled { "enabled" } else { "disabled" });
    Ok(())
}
