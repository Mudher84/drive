...
pub struct TelegramState {
    ...
    /// Send to this channel to stop the background update listener that powers
    /// desktop notifications for incoming Telegram messages. Mirrors the
    /// `runner_shutdown` pattern: signalled (and replaced) on logout/reconnect.
    pub notifier_shutdown: Arc<std::sync::Mutex<Option<tokio::sync::oneshot::Sender<()>>>>,
